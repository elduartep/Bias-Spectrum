...
//OPENMP
//you need to install openmp
#include <omp.h>

//you need to install FFTW3
//FFTW
#include "fftw3.h"
...
//numerical recipes
//this program usses some numerical recipes
#include "nr3.h"
#include "gamma.h"
...
//cosmological and other parameters you need to specify
#include "parametros.h"


//this is how you compile this code
//g++ Bias_pragma_lb.c -lfftw3_omp -lfftw3 -lm -fopenmp
//and then just type ./a.out to run it
...
const double k_lineal=0.25;             //      k_max in h/Mpc for computing linear bias
...
const int    bin=64;              	//	number of bins for printing the power spectrum
const int    binb=8;              	//	number of bins used for computing the linear bias
...
double *x;			//	auxiliar for rsd
double *vx;			//	auxiliar for rsd
double *MP, *Mk, *EP, *Ek;	//      for computing the mean of the power spectrum
double *MP2, *EP2;		//      for computing the mean of the cross power spectrum
double *MPb, *EPb;

double *X, *Y, *Z, *R;		//	position and radius for voids

double *MPb2, *EPb2;		//      for computing the mean of the cross power spectrum
double *MPb3, *EPb3;		//      for computing the mean of the cross power spectrum
double *MPb4, *EPb4;		//      for computing the mean of the cross power spectrum
double *MPb5, *EPb5;		//      for computing the mean of the cross power spectrum
double *MPb6, *EPb6;		//      for computing the mean of the cross power spectrum
long int *cont;			//      for computing the mean of the cross power spectrum
long int *cont2;		//	for computing the mean of the cross power spectrum
double *carga;			//	average mass or radius in each bin
double *Ecarga;			//	variance masa or radio in each bin
int    *cantidad;		//	number of halos or voids in each bin
int    *fuera;			//	outside or inside the voids
double cmax,cmin,c;		//	para los bines binb de masa/radio
int NumVoids;			//	number of voids
int NumHalos;			//	number of halos
...
void max_min_c(const char *NomArch){
  ...
  IN=fopen(NomArch,"r");
  if (IN == NULL) {
    printf("Unable to open file_halos = %s\n",NomArch);
    exit(0);
  }
  NumHalos=0;
...
omp_set_num_threads(nt);
  ...
  if(bias_halos==1){
    sprintf(ArchEn,"%s",file_halos);
    max_min_c(ArchEn);

    if (puntual==1)
      sprintf(ArchEn,"%s",file_halos);			//	puntual
    else
      sprintf(ArchEn,"particles_%s",file_halos);	//	continuo
    sprintf(ArchSa,"bias_halo_matter.dat");
  ...
  if(bias_voids==1){
    if((puntual==1)&&(solo_analiza_bias_voids==0))
      sprintf(ArchEn,"%s",file_voids);                  //      punctual
    else if (solo_analiza_bias_voids==0)
      sprintf(ArchEn,"particles_%s",file_voids);	//	continuous
    sprintf(ArchSa,"bias_void_matter.dat");
  ...
  if(espectro_voids==1){
    if(puntual==1)
      sprintf(ArchEn,"%s",file_voids);			//	puntual
    else
      sprintf(ArchEn,"particles_%s",file_voids);	//	continuo
    sprintf(ArchSa,"Pv.dat");
